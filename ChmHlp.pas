...
type
  ...
  function HtmlHelpContext(iHelpContext: Integer): HWND;

implementation
uses Forms, SysUtils;

type
  TCHMHelpHandler = class
  public
    hwndHlp: HWND;
    function HandleHelp(Command: Word; Data: Longint; var CallHelp: Boolean): Boolean;
  end;

var
  CHMHelpHandler: TCHMHelpHandler;
  ...
  function HtmlHelpContext(iHelpContext: Integer): HWND;
  begin
    Result := HtmlHelp(0, PChar(Application.CurrentHelpFile), HH_HELP_CONTEXT, DWORD(iHelpContext));
  end;

   // TCHMHelpHandler

  function TCHMHelpHandler.HandleHelp(Command: Word; Data: Integer; var CallHelp: Boolean): Boolean;
  begin
    if GetKeyState(VK_Shift)<0 then Application.MessageBox(PChar(Format('ֲûחמג Help: Command=%d, ID=%d', [Command, Data])), 'Help Debugging');
...
initialization
  CHMHelpHandler := TCHMHelpHandler.Create;
  Application.OnHelp := CHMHelpHandler.HandleHelp;
finalization
  CHMHelpHandler.Free;
end.
